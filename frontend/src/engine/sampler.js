...
function evalBound(val, env, userFns) {
    return evaluate(val, env, userFns);
}
function xInDomain(x, constraints, env, userFns) {
    if (!constraints)
        return true;
    for (const c of constraints) {
        ...
        if (c.lo) {
            ...
            if (c.lo.inclusive ? x < lo : x <= lo)
                return false;
        }
        if (c.hi) {
            const hi = evalBound(c.hi.value, env, userFns);
            if (c.hi.inclusive ? x > hi : x >= hi)
                return false;
        }
    }
    return true;
}
function yInRange(y, constraints, env, userFns) {
    if (!constraints)
        return true;
    for (const c of constraints) {
        if (c.variable !== "y")
        ...
        if (c.lo) {
            const lo = evalBound(c.lo.value, env, userFns);
            if (c.lo.inclusive ? y < lo : y <= lo)
                return false;
        }
        if (c.hi) {
            const hi = evalBound(c.hi.value, env, userFns);
            if (c.hi.inclusive ? y > hi : y >= hi)
                return false;
        }
    }
    return true;
}
function bisectDomainBoundary(x0, x1, in0, constraints, env, userFns) {
    for (let i = 0; i < 12; i++) {
        const mid = (x0 + x1) / 2;
        if (xInDomain(mid, constraints, env, userFns) === in0)
            x0 = mid;
        else
            x1 = mid;
    }
...
function adaptiveRefine(fn, x0, x1, y0, y1, yRange, depth, out) {
    if (depth === 0) {
        out.push(x1, y1);
        return;
    }
...
function sampleCartesian(fn, viewport, baseN = 600, constraints, env, userFns) {
...
function sampleParametric(xFn, yFn, tMin = 0, tMax = 2 * Math.PI, n = 800) {
    const out = [];
    for (let i = 0; i <= n; i++) {
...
function samplePolar(rFn, thetaMin = 0, thetaMax = 2 * Math.PI, n = 800) {
    const out = [];
    for (let i = 0; i <= n; i++) {
...
function sampleImplicit(f, viewport, res = 200) {
...
export function sampleExpression(parsed, env, viewport, userFns = {}) {
    const baseEnv = { ...env };
    ...
    switch (parsed.type) {
        case "function": {
            ...
            if (param === "θ" || param === "theta") {
                const points = samplePolar((theta) => evaluate(body, { ...baseEnv, [param]: theta }, userFns));
                return { points, kind: "polar" };
            }
            if (param === "t") {
                const points = sampleCartesian((t) => evaluate(body, { ...baseEnv, t }, userFns), { ...viewport, xMin: 0, xMax: 2 * Math.PI }, 600, constraints, baseEnv, userFns);
                return { points, kind: "cartesian" };
            }
            const points = sampleCartesian((x) => evaluate(body, { ...baseEnv, [param]: x }, userFns), viewport, 600, constraints, baseEnv, userFns);
            return { points, kind: "cartesian" };
        }
        case "expression": {
            const body = parsed.body;
            const points = sampleCartesian((x) => evaluate(body, { ...baseEnv, x }, userFns), viewport, 600, constraints, baseEnv, userFns);
            return { points, kind: "cartesian" };
        }
        case "equation": {
            const { lhs, rhs } = parsed;
            if (rhs.kind === "variable" && rhs.name === "y") {
                const points = sampleCartesian((x) => evaluate(lhs, { ...baseEnv, x }, userFns), viewport, 600, constraints, baseEnv, userFns);
                return { points, kind: "cartesian" };
            }
            if (lhs.kind === "variable" && lhs.name === "y") {
                const points = sampleCartesian((x) => evaluate(rhs, { ...baseEnv, x }, userFns), viewport, 600, constraints, baseEnv, userFns);
                return { points, kind: "cartesian" };
            }
            const f = (x, y) => evaluate(lhs, { ...baseEnv, x, y }, userFns) -
...
export function sampleWithLists(parsed, env, viewport, userFns, listAssignments) {
...
export function sampleParametricPair(xBody, yBody, env, tMin = 0, tMax = 2 * Math.PI, userFns = {}) {
    const points = sampleParametric((t) => evaluate(xBody, { ...env, t }, userFns), (t) => evaluate(yBody, { ...env, t }, userFns), tMin, tMax);
