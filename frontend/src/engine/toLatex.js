...
function varToLatex(name) {
    if (name in GREEK_TO_LATEX)
        return GREEK_TO_LATEX[name];
    if (name.length > 2)
        return `\\text{${name}}`;
    return name;
}
...
const PREC_MUL = 2;
const PREC_POW = 3;
function canImplicitMul(left, right) {
...
// ── Core AST → LaTeX ─────────────────────────────────────────────────────────
function wrapIfNeeded(latex, prec, minPrec) {
    return prec < minPrec ? `\\left(${latex}\\right)` : latex;
}
function astToLatex(node, minPrec = 0) {
    switch (node.kind) {
        case "number": {
            const v = node.value;
            if (!isFinite(v))
...
function callToLatex(node) {
    ...
    if (fn === "sqrt") {
        ...
        if (args.length === 2)
            return `\\sqrt[${astToLatex(args[1])}]{${astToLatex(args[0])}}`;
    }
    if (fn === "abs")
        return `\\left|${astToLatex(args[0])}\\right|`;
    if (fn === "exp")
        return `e^{${astToLatex(args[0])}}`;
    if (fn === "floor")
        return `\\lfloor ${astToLatex(args[0])} \\rfloor`;
    if (fn === "ceil")
        return `\\lceil ${astToLatex(args[0])} \\rceil`;
    const latexName = TRIG_FNS[fn] ?? `\\operatorname{${fn}}`;
    const argsStr = args.map(a => astToLatex(a)).join(", ");
    return `${latexName}\\left(${argsStr}\\right)`;
}
// ── Constraint rendering ──────────────────────────────────────────────────────
function constraintsToLatex(cs) {
    if (cs.length === 0)
        return "";
    const parts = cs.map(c => {
        ...
        const pieces = [];
        if (c.lo)
            pieces.push(`${astToLatex(c.lo.value)} ${c.lo.inclusive ? "\\leq" : "<"} ${v}`);
        if (c.hi)
            pieces.push(`${v} ${c.hi.inclusive ? "\\leq" : "<"} ${astToLatex(c.hi.value)}`);
        // If only one side parsed, show it
        if (pieces.length === 2)
            return `${astToLatex(c.lo.value)} ${c.lo.inclusive ? "\\leq" : "<"} ${v} ${c.hi.inclusive ? "\\leq" : "<"} ${astToLatex(c.hi.value)}`;
        return pieces.join(",\\ ");
    });
    return `\\left\\{${parts.join(",\\ ")}\\right\\}`;
}
// ── Public API ────────────────────────────────────────────────────────────────
export function toLatex(parsed) {
    switch (parsed.type) {
        case "function": {
