...
import { num, vari, cnst, binop, call } from "../ast";
...
describe("evaluate — binary ops", () => {
    ...
    it("unknown op returns NaN without throwing", () => {
        const node = { kind: "binary", op: "%", left: num(2), right: num(3) };
    ...
    it("unknown op does NOT fall through to call case", () => {
        ...
        const spy = [];
        const node = { kind: "binary", op: "%", left: num(1), right: num(2) };
        const userFns = {
            __probe__: (args) => { spy.push("called"); return args[0]; },
        };
...
describe("evaluate — higher-order derivatives use correct paramVar", () => {
    ...
    it("g''(t) with slider 'k' first in env uses t not k", () => {
        const userFns = {
            g: (args) => args[0] ** 3, // g(t) = t^3, g''(t) = 6t
        };
        const env = { k: 99, t: 2 };
