...
export function evaluate(node, env, userFns = {}) {
    switch (node.kind) {
        case "number":
            return node.value;
        case "constant":
        ...
        case "call": {
            ...
            if (node.fn.startsWith("__prime__")) {
                ...
                const callInner = (xv) => {
                    ...
                    if (ufn2)
                        return ufn2([xv, ...args.slice(1)]);
                    const bfn = BUILTINS[inner];
                    if (bfn)
                        return bfn(xv, ...args.slice(1));
                    return NaN;
                };
                const h = 1e-7;
                const x0 = args[0] ?? NaN;
        ...
        case "piecewise": {
            for (const branch of node.branches) {
                ...
                if (cond !== 0 && !Number.isNaN(cond))
                    return evaluate(branch.value, env, userFns);
            }
...
export function evaluateBool(node, env, userFns = {}) {
