...
const UTILS = {
    linspace: (start, end, n) => {
        const arr = [];
        for (let i = 0; i < n; i++)
            arr.push(start + (i / (n - 1)) * (end - start));
        return arr;
    },
 ...
 */
export async function runSandboxed(js, inputs, ctx) {
    try {
        ...
        const factory = new Function("inputs", "ctx", "Math", "utils", "globalThis", "window", "document", "self", "location", "fetch", "XMLHttpRequest", `"use strict";\n${js}\nreturn run(inputs, ctx);`);
        const result = await Promise.resolve(factory(inputs, ctx, SAFE_MATH, UTILS, undefined, undefined, undefined, undefined, undefined, undefined, undefined));
        // Validate shape
