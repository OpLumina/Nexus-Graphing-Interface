...
export type FrontendOpFn = (
  args: Record<string, unknown>,
  ctx: ToolContext,
) => Record<string, unknown>;

function buildEvalAt(ctx: ToolContext): ((x: number) => number) | null {
...
export const FRONTEND_OPS: Record<string, FrontendOpFn> = {

  evaluate_table: (_args, ctx) => {
    const evalAt = buildEvalAt(ctx);
    if (!evalAt) return { rows: [] };
    const xs = [-2, -1, 0, 1, 2];
  ...
  evaluate_assignment: (_args, ctx) => {
    const expr = ctx.expressions.find(e => e.id === ctx.exprId);
    if (!expr || expr.parsed.type !== "assignment") return { value: NaN };
