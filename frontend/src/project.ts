...
import { ExpressionEntry, SliderEntry } from "./store";
import { ViewportState } from "./renderer/viewport";
...
export function serialise(data: ProjectData): string {
  const lines: string[] = [];
...
export function deserialise(text: string): ProjectData | null {
  try {
    ...
    for (; i < lines.length; i++) {
      ...
      if (line.startsWith("# ")) { flushExpr(); continue; }
      if (!line.trim()) continue;

      if (current) {
        const colorM = line.match(/^color:\s*(.+)/);
        if (colorM) { current.color = colorM[1].trim(); continue; }
        const visM = line.match(/^visible:\s*(.+)/);
        if (visM) { current.visible = visM[1].trim() === "true"; continue; }
        const exprM = line.match(/^expr:\s*(.*)/);
        if (exprM) { current.raw = exprM[1]; continue; }
      } else {
