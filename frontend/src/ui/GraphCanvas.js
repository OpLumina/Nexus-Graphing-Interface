...
export function GraphCanvas() {
    ...
    const mouseDownPos = useRef([0, 0]);
    const reflectTimerRef = useRef();
    const sampledCachRef = useRef(new Map());
    ...
    useEffect(() => {
        ...
        drawAxes(ctx2d, viewport);
        {
            ...
            for (const expr of expressions) {
                if (!expr.visible || expr.parsed.type !== "inequality")
                    continue;
                ...
                for (let ci2 = 0; ci2 < COLS; ci2++) {
                    ...
                    for (let ri = 0; ri < ROWS; ri++) {
                        ...
                        const holds = op === "<" ? lv < rv :
                            op === ">" ? lv > rv :
                                op === "<=" ? lv <= rv :
                                    lv >= rv;
                        if (holds)
                            ctx2d.rect(ci2 * cw, ri * ch, cw, ch);
                    }
                }
                ctx2d.fill();
            }
        ...
        for (const expr of expressions) {
            ...
            curves.forEach((curve, ci) => {
                ...
                if (ci === 0)
                    newCache.set(expr.id, curve.points);
            });
            colorIndex++;
        }
        sampledCachRef.current = newCache;
        const allOverlays = Object.values(toolResults)
            .flatMap(r => r.overlays ?? []);
        for (const overlay of allOverlays) {
            if (!isFinite(overlay.slope) || !isFinite(overlay.y0))
                continue;
    ...
    return (_jsxs("div", { ref: containerRef, style: { flex: 1, position: "relative", overflow: "hidden" }, children: [_jsx("canvas", { ref: glCanvasRef, style: { position: "absolute", inset: 0, width: "100%", height: "100%" }, onMouseDown: onMouseDown, onMouseMove: onMouseMove, onMouseUp: onMouseUp, onMouseLeave: () => { isDragging.current = false; }, onWheel: onWheel }), _jsx("canvas", { ref: axisCanvasRef, style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" } }), inspectPoint && (_jsxs("div", { style: {
                    position: "absolute",
                ...
                }, children: [_jsx("span", { style: { color: inspectPoint.color, marginRight: 6 }, children: "\u25CF" }), _jsx("span", { style: { color: "#aaa" }, children: "x" }), " = ", _jsx("span", { style: { color: "#e8e8e8" }, children: inspectPoint.mathX.toFixed(3) }), "  ", _jsx("span", { style: { color: "#aaa" }, children: "y" }), " = ", _jsx("span", { style: { color: "#e8e8e8" }, children: inspectPoint.mathY.toFixed(3) })] }))] }));
}
