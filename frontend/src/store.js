...
import { freeVariables } from "./engine/ast";
import { evaluate } from "./engine/evaluator";
import { defaultViewport } from "./renderer/viewport";
import { buildDependencyGraph } from "./engine/dependency";
...
function deriveSliders(expressions, existing) {
    const defined = new Set();
    const used = new Set();
    for (const e of expressions) {
        if (e.parsed.type === "function")
            defined.add(e.parsed.name);
...
export const useStore = create((set, get) => ({
    ...
    removeExpression: (id) => {
        set(state => {
            ...
            const sliders = deriveSliders(expressions, state.sliders);
            const toolResults = Object.fromEntries(Object.entries(state.toolResults).filter(([k]) => !k.endsWith(`:${id}`)));
            return { expressionHistory, expressions, sliders, depGraph: rebuildDepGraph(expressions), toolResults };
        });
    },
    toggleVisibility: (id) => set(state => ({ expressions: state.expressions.map(e => e.id === id ? { ...e, visible: !e.visible } : e) })),
    toggleOutput: (id) => set(state => ({ expressions: state.expressions.map(e => e.id === id ? { ...e, showOutput: !e.showOutput } : e) })),
    toggleGlobal: (id) => set(state => ({ expressions: state.expressions.map(e => e.id === id ? { ...e, isGlobal: !e.isGlobal } : e) })),
    setColor: (id, color) => set(state => ({ expressions: state.expressions.map(e => e.id === id ? { ...e, color } : e) })),
    undo: () => {
        set(state => {
    ...
    setSliderValue: (name, value) => set(state => ({ sliders: state.sliders.map(s => s.name === name ? { ...s, value } : s) })),
    setSliderRange: (name, min, max) => set(state => ({ sliders: state.sliders.map(s => s.name === name ? { ...s, min, max } : s) })),
