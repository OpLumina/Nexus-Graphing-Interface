...
export class CurveRenderer {
  ...
  private locViewport: WebGLUniformLocation;
  ...
  drawCurve(points: Float32Array, viewport: ViewportState, style: CurveStyle): void {
    const { gl } = this;
...
export const CURVE_COLORS: [number, number, number][] = [
  [0.38, 0.68, 1.00],
  [1.00, 0.42, 0.42],
  [0.42, 0.88, 0.56],
  [1.00, 0.78, 0.22],
...
export function colorForIndex(i: number): [number, number, number] {
