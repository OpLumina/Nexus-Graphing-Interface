import { viewportWidth, viewportHeight } from "./viewport";
const DEFAULTS = {
...
function niceStep(range, pixels, minPxPerTick = 60) {
    const approx = (range / pixels) * minPxPerTick;
    const mag = Math.pow(10, Math.floor(Math.log10(approx)));
    for (const factor of [1, 2, 5, 10]) {
        if (mag * factor >= approx)
            return mag * factor;
    }
    return mag * 10;
}
export function drawAxes(ctx, viewport, opts = {}) {
    ...
    const toPixX = (mx) => ((mx - xMin) / mathW) * W;
    const toPixY = (my) => ((yMax - my) / mathH) * H;
